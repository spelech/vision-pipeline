...
class AdvancedPipeline(BasePipeline):
    ...
    def run(self, image=None, text_description=None, settings=None, log_cb=None):
        results = {"barcode": None, "llm_output": None, "searxng_results": [], "scraped_content": None}

        # 1. Barcode & Initial LLM
        if image: results["barcode"] = scan_barcode(image, log_cb=log_cb)
        
        model = settings.get("vision_model", "qwen/qwen2.5-vl-72b-instruct") if settings else "qwen/qwen2.5-vl-72b-instruct"
        results["llm_output"] = vision_identify(image, text_description, model=model, log_cb=log_cb)
        ...
        if query and query not in ["Unknown", "Error"]:
            ...
            if best_url:
                wait_time = settings.get("scrape_wait_time", 2000) if settings else 2000
                results["scraped_content"] = web_scrape(best_url, wait_time=wait_time, log_cb=log_cb)
            ...
            if results["searxng_results"] or results["scraped_content"]:
                ...
                results["llm_output"] = data_refine(results["llm_output"], context, log_cb=log_cb)

        if log_cb: log_cb("🏁 Advanced Pipeline finished.")
        return results
